...
void AutoRouter::calculate(const std::vector<Node *> &nodes,
    ...
    for(Node *node : nodes){
      ...
      if(host == nullptr && nat == nullptr) continue;
      
      Address destAddress = (host == nullptr) ? nat->get_address() : host->address();

      
      // 도착 정점의 node index
      int next_index = node_to_index[node];
      ...
      for(Link *link : adj[cur]){
        if(next_node == link->other(this)){
          routingTable_.push_back(RoutingEntry(destAddress, link));
          break;
        }
      }
    }
